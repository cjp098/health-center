...
function Patients() {
    ...
    const toComplaints = (event, id) => {
        ...
        history.push(`/complaints?id=${id}`)
    }

    const toUpdateInfo = (event, id) => {
        event.preventDefault();

        history.push(`/updatePatient?id=${id}`)
    }
        ...
        },
        {
            ...
            render: (patient) => (
                <Space size="middle" key="action">
                    ...
                    >
                        complaints{" "}
                        <string className="bg-blue-300 rounded-full py-0.5 px-1.5">
                            {patient.diagnosis.length}
                        </string>
                    </Popconfirm>
                    <Popconfirm
                        title="would you like to continue?"
                        onConfirm={(event) => toUpdateInfo(event, patient.id)}
                        className="cursor-pointer  bg-green-500 hover:bg-green-400 py-2 px-3 rounded-sm shadow-lg text-white"
                    >
                        update
                    </Popconfirm>
                </Space>
